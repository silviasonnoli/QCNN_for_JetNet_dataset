...
import h5py
import hdf5plugin
from jetnet.datasets.dataset import JetDataset
from jetnet.datasets.normalisations import NormaliseABC
from jetnet.datasets.utils import (
...
class TopTaggingPatch:
    ...
    Args:
        ...
        jet_transform (callable, optional): A function/transform that takes in the jet
            data tensor tensor and transforms it. Defaults to None.
    ...
    def getData(
        ...
        Args:
            ...
            num_particles (int, optional): number of particles to retain per jet, max of 200.
                Defaults to 200.
            indices (np.ndarray, optional): subset of indices to retrieve. Defaults to None.
            split (str, optional): dataset split, out of {"train", "valid", "test", "all"}. Defaults
                to "all".
            download (bool, optional): If True, downloads the dataset from the internet and
                puts it in the ``data_dir`` directory. If dataset is already downloaded, it is not
        ...
        for s in split:
            ...
            with h5py.File(hdf5_file, "r") as f:
                # Assuming "table" is a group and the dataset is also named "table" inside it
                dataset = f["table"]["table"]
                if indices is not None:
                    # Ensure indices are unique and sorted in increasing order
                    sorted_unique_indices = np.unique(indices)
                    # Access the dataset within the group and apply fancy indexing
                    data = dataset[sorted_unique_indices]
                else:
                    # If indices is None, load the entire dataset from within the group.
                    # This still has potential memory issues for very large files.
                    data = dataset[:]


                data = np.void(data)
                data, jet_type_ = data[:]['values_block_0'], data[:]['values_block_1']
            ...
            jet_selector = np.sum([[jet_type_[j][-1] == i for i in type_indices] for j in range(len(jet_type_))], axis=1).astype(bool)#[:, -1]
            ...
            if use_jet_features:
                jf = np.concatenate(
                    (jet_type_[:, -1:], data[:, total_particle_features : total_particle_features + 4]),
                    axis=-1,
                )
